...
- Remaining real suspects (next): 8642666611469255 (69/666), 520698390475976 (-311), soa_ (-222), 8855611820 (-165), Bank statement (2)/(3) (-39 ea), shivlal (-13), KOMAL (-6), DEVANSHU (-6).

### 2026-07-04 — Session 14: UCO Bank fix, both files exact-match verified
- `AccountStmt_0882.../1228...pdf` (UCO Bank): overlapping letterhead corrupts the debit/credit table columns; the transaction AMOUNT wraps onto its own bare continuation line while the running BALANCE stays reliably on the date line. Rather than guess which of two ambiguous numbers is the real amount, built `read_uco_style_lines()` — derives amount+direction from the balance delta between consecutive rows, gated narrowly by an exact header-text fingerprint so it can never fire on any of the other 160 working files.
- Two bugs caught before landing: (1) balance chain was resetting to None at every page boundary (13 pages → 13 silently dropped transactions) — fixed by concatenating all pages into one text stream before parsing; (2) the genuine "Opening Balance as of ... 0.00 CR" line doesn't match the transaction regex, so the code was using the FIRST REAL transaction as the balance seed and silently discarding it — fixed with a dedicated opening-balance regex.
- Verified against the bank's own printed GRAND TOTAL line (independent ground truth, not our own math): `AccountStmt_0882` — extracted debit ₹16,46,385.90 / credit ₹16,56,943.00 / closing ₹10,557.10, **all three match the statement's GRAND TOTAL exactly**, 0 balance breaks, 221 rows (was 2). `AccountStmt_1228`: 7 → 218 rows, 0 breaks.
...
- Remaining: 520698390475976 (partial, see below), soa_, 8855611820, Bank statement (2)/(3), shivlal, KOMAL, DEVANSHU.

### 2026-07-04 — Session 16: 520698390475976 — clean win + one honest known limitation
- Root-caused a "N 1.00"-style bug: a wrapped narration fragment ('N' from 'NEFT...') bleeding into the adjacent amount cell, silently failing `parse_amount` → row dropped entirely. Fixed generally in `parse_amount` (strip a leading 1-3 letter prefix only when what remains is an unambiguous complete number) — benefits any file with this artifact, not just this one. 3486→3511 rows. 74/74 tests (1 new), no regressions on 8642666611469255/TARUN/AccountStmt_0882.
- **Investigated but NOT fixed, documented honestly**: 20 of this file's 111 pages (18 at 8 columns, 2 at 9) have a shifted table column-count vs the standard 7. On those pages debit/credit amounts land in unpredictable positions and the true running balance vanishes for ~286 rows. Recovering this correctly needs cross-row balance triangulation (last-known-good balance before the shift ↔ next-known-good after it) — real design work, not a targeted patch. Decided NOT to hand-hack a quick fix here: guessing wrong would silently assign incorrect amounts to real transactions, which is worse for evidence integrity than the current behavior (those rows are cleanly skipped, not silently wrong). Flagging as a known architectural limitation for a future session, not a bug to rush.
- Current: 3511/3797 rows (~92.5%), 2 balance breaks remaining (both inside the misaligned-column region, expected).

### 2026-07-04 — Session 17: soa_ verified already-complete; 8855611820 fixed with a near-miss regression caught in time
- `soa_0167042251865512.pdf`: the "222 missing" from the completeness audit was my OWN estimator's bug — it counted a page-header artifact line (`11/28/25, 3:40 PM blob:...`) as a transaction on every one of 222 pages, exactly matching the reported gap. File is verified 100% complete, no code change needed.
- `8855611820.pdf` (Kotak Mahindra, 90 pages): a broader version of the fake-table bug — pdfplumber traps a lone wrapped-narration FRAGMENT (no date, no amount, e.g. `'GHOSH/412279502997/UP'`) in a spurious single-cell table on 10/90 pages. The existing fake-table detector only recognized fakes containing a FULL transaction line, so fragments slipped through, the page's real table stayed "trusted," and everything outside the tiny fake table was silently discarded.
- **First attempt regressed real files** (self-caught before committing): broadening to "any lone-populated-cell row is fake" fixed this file (1123→1269) but SILENTLY DROPPED rows on 3 previously-fixed files (001029700065_SOA 347→324, 520698390475976 3511→1873 rows lost, statement-38347344323 12087→11899) — customer-info cells (email, plain account number, footer balance) share the same "lone cell, no date, no amount" shape as a real fragment, and the broad rule couldn't tell them apart.
- **Correct, narrow fix**: a lone cell is only a "fragment" when it also carries a bank-transfer marker (`/`, `UPI`, `IMPS`, `NEFT`, etc.) — narrations look like this, customer-info cells never do. Re-verified against all 10 previously-fixed/known-good files: every one matches its exact prior baseline, and `8855611820` now hits **1269/1269 rows, 0 breaks** (was 1123).
- Lesson for the remaining files: always re-run the FULL prior-fixes regression set before committing any shared-code change, not just the target file — this is now standard practice for the rest of the queue. 75/75 tests (1 new).
### 2026-07-04 — Session 18: suspect-list QUEUE CLEARED (one shot, as requested)
- **Bank statement...(2)/(3).pdf**: real bug. Credit rows carry a phantom extra cell (pdfplumber splitting a wrapped/spaced deposit value) that debit rows don't — so credit amounts land one column right of the mapped position. Fixed generally in `grid_to_txns`: when mapped debit+credit are both empty, check one column past the mapped credit index for a valid amount+following-balance pair. **374→413 rows, 0 breaks (exact match), both files.** Bonus: same fix recovered +210 rows on `520698390475976` too (3511→3721).
- **shivlal statement.txt**: real bug, different cause. A long branch-address narration ("SARDARPURA JODHPUR \\") pushes the ATM-withdrawal amount out of its mapped fixed-width column into an unmapped trailing cell. First attempt (default any orphan amount to DEBIT) fixed this file but silently corrupted `520698390475976`'s already-fragile misaligned-column rows (3→43 breaks) — caught by the mandatory full-regression-check-before-commit habit, reverted, and re-scoped to only fire when the row's own text unambiguously signals a withdrawal (ATM/WDR/CASH keyword) — direction confirmed by content, never guessed. **357→369 rows, 0 breaks**, no regression anywhere.
- **soa_0167042251865512.pdf, KOMAL statement.pdf, DEVANSHU_STMNT.pdf: all three verified ALREADY COMPLETE, zero code changes needed.** Every one of their "missing" counts was my own completeness-audit estimator counting non-transaction content as if it were a transaction: soa_'s was a page-header URL artifact (blob: link) on all 222 pages; KOMAL's were 6 page-header bank-name+timestamp lines; DEVANSHU's were 2 header timestamps plus 4 rows from a completely separate interest-rate/limits schedule table elsewhere in the report. In every case, (doc-estimate − non-transaction lines) = our extracted count, exactly.
