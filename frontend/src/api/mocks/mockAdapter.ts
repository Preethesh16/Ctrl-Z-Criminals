...
const signatures = new Map<
  string,
  { verify_id: string; signature: string; signed_at: string; case_id: string; report_type: string; content_hash: string }
>()
...
export const mockAdapter = {
  ...
  /* Digital signatures — mock keeps records in memory for the verify flow. */
  async signReport(caseId: string, reportType: string, contentHash: string) {
    await delay(150)
    const verify_id = nextId('sig')
    const record = {
      verify_id,
      signature: seededRandom(`${verify_id}${contentHash}`)()
        .toString(16)
        .slice(2)
        .padEnd(16, 'a')
        .repeat(4)
    ...
    signatures.set(verify_id, { ...record, content_hash: contentHash })
    return record
  },

  async verifyReport(verifyId: string) {
    await delay(150)
    const token = verifyId.trim().replace(/[….\s]+$/, '')
    const rec =
      signatures.get(token) ??
      (token.length >= 8
        ? [...signatures.values()].find(
            (r) => r.signature.startsWith(token) || r.content_hash.startsWith(token),
          )
        : undefined)
    if (rec) {
      const c = cases.get(rec.case_id)
      return {
        valid: true,
        case_id: rec.case_id,
        fir_number: c?.fir_number ?? null,
        report_type: rec.report_type,
        content_hash: rec.content_hash,
        signed_at: rec.signed_at,
      }
    }
    // evidence-locker hash: match an uploaded document's sha256
    const doc =
      token.length >= 8
        ? [...documents.values()].find((d) => d.sha256.startsWith(token))
        : undefined
    if (doc) {
      const c = cases.get(doc.case_id)
      return {
        valid: true,
        case_id: doc.case_id,
        fir_number: c?.fir_number ?? null,
        report_type: `source statement in the evidence locker: ${doc.filename}`,
        content_hash: doc.sha256,
        signed_at: new Date().toISOString(),
      }
    }
    throw new ApiError(404, 'no such report token — this report was NOT generated by this system')
  },
}
