...
def verify_report(verify_id: str, db: Session = Depends(get_db)):
    """Look a report up by ANY token printed in its footer — the Verification
    ID, the signature, or the content hash — and re-check its signature.

    valid=True  → the report was generated by this system and is untampered.
    404         → no such record: the document is not ours (or the token is wrong).
    """
    from .models import Document, ReportSignature

    # accept the verification ID, or a signature/content-hash (possibly the
    # truncated "…" form printed in the footer) copied from the report
    token = verify_id.strip().rstrip("….")
    row = db.get(ReportSignature, token)
    if row is None and len(token) >= 8:
        stmt = select(ReportSignature)
        row = db.scalar(stmt.where(ReportSignature.signature.like(f"{token}%"))) or db.scalar(
            stmt.where(ReportSignature.content_hash.like(f"{token}%"))
        )
    if row is not None:
    ...
    # not a report signature — is it an evidence-locker hash? The investigation
    # report prints the SHA-256 of every source statement in its evidence chain.
    doc = None
    if len(token) >= 8:
        doc = db.scalar(select(Document).where(Document.sha256.like(f"{token}%")))
    if doc is not None:
        ...
        return {
            ...
            "case_id": doc.case_id,
            "fir_number": case.fir_number if case else None,
            "report_type": f"source statement in the evidence locker: {doc.filename}",
            "content_hash": doc.sha256,
            "signed_at": doc.uploaded_at.isoformat(),
        }

    raise HTTPException(404, "no such report token — this report was NOT generated by this system")
